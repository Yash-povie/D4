"use client"

import { useState } from "react"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Progress } from "@/components/ui/progress"
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog"
import { Crown, AlertTriangle, RotateCcw, XCircle, Undo2 } from "lucide-react"

interface CurrentPlan {
  planName: string
  price: string
  nextBilling: string
  status: string
  stripeStatus: string
  cancelAtPeriodEnd?: boolean
  daysUsed: number
  totalDays: number
  progressPercentage: number
  remainingDays: number
  needsAttention: boolean
  attentionMessage: string
}

interface CurrentPlanCardProps {
  plan: CurrentPlan
  onResubscribe?: () => void
  onCancel?: () => Promise<void>
  onResume?: () => Promise<void>
}

function getStatusBadgeVariant(stripeStatus: string) {
  switch (stripeStatus) {
    case "active":
    case "trialing":
      return "secondary" as const
    case "canceling":
      return "outline" as const
    case "canceled":
    case "past_due":
    case "unpaid":
      return "destructive" as const
    default:
      return "outline" as const
  }
}

export function CurrentPlanCard({ plan, onResubscribe, onCancel, onResume }: CurrentPlanCardProps) {
  const isCanceled = plan.stripeStatus === "canceled"
  const isCanceling = plan.stripeStatus === "canceling" || plan.cancelAtPeriodEnd
  const isActive = plan.stripeStatus === "active" && !plan.cancelAtPeriodEnd
  const isTrial = plan.stripeStatus === "trialing"
  const [cancelDialogOpen, setCancelDialogOpen] = useState(false)
  const [canceling, setCanceling] = useState(false)
  const [resuming, setResuming] = useState(false)

  const handleCancel = async () => {
    if (!onCancel) return
    setCanceling(true)
    try {
      await onCancel()
      setCancelDialogOpen(false)
    } catch {
      // error handled by parent
    } finally {
      setCanceling(false)
    }
  }

  const handleResume = async () => {
    if (!onResume) return
    setResuming(true)
    try {
      await onResume()
    } catch {
      // error handled by parent
    } finally {
      setResuming(false)
    }
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle>Current Plan</CardTitle>
        <CardDescription>
          {isCanceled
            ? "Your subscription has been canceled."
            : isCanceling
              ? "Your subscription is scheduled for cancellation."
              : `You are currently on the ${plan.planName}.`}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            <Crown className="h-5 w-5 text-yellow-500" />
            <span className="font-semibold">{plan.planName}</span>
            <Badge variant={getStatusBadgeVariant(plan.stripeStatus)}>{plan.status}</Badge>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold">{plan.price}</div>
            <div className="text-sm text-muted-foreground">
              {isCanceled || isCanceling ? plan.nextBilling : `Next billing: ${plan.nextBilling}`}
            </div>
          </div>
        </div>

        {plan.needsAttention && (
          <Card className="border-neutral-200 bg-neutral-50 dark:border-neutral-600 dark:bg-neutral-800">
            <CardContent>
              <div className="flex items-start gap-3">
                <AlertTriangle className="h-5 w-5 text-neutral-600 mt-0.5 dark:text-neutral-400" />
                <div className="space-y-1">
                  <p className="font-medium text-neutral-800 dark:text-neutral-400">
                    {isCanceled
                      ? "Subscription Canceled"
                      : isCanceling
                        ? "Cancellation Pending"
                        : "We need your attention!"}
                  </p>
                  <p className="text-sm text-neutral-700 dark:text-neutral-400">{plan.attentionMessage}</p>
                </div>
              </div>

              {/* Progress Section */}
              <div className="mt-4 space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground font-medium">Days</span>
                  <span className="text-sm text-muted-foreground font-medium">{plan.daysUsed} of {plan.totalDays} Days</span>
                </div>
                <Progress value={plan.progressPercentage} className="h-2" />
                <p className="text-xs text-muted-foreground">
                  {isCanceled
                    ? `${plan.remainingDays} days of access remaining`
                    : isCanceling
                      ? `${plan.remainingDays} days until cancellation takes effect`
                      : `${plan.remainingDays} days remaining until your plan requires update`}
                </p>
              </div>

              {/* Undo cancellation for pending cancel */}
              {isCanceling && onResume && (
                <Button
                  className="w-full mt-4 cursor-pointer"
                  onClick={handleResume}
                  disabled={resuming}
                >
                  <Undo2 className="size-4 mr-2" />
                  {resuming ? "Resuming..." : "Undo Cancellation"}
                </Button>
              )}

              {/* Resubscribe for fully canceled */}
              {isCanceled && onResubscribe && (
                <Button
                  className="w-full mt-4 cursor-pointer"
                  onClick={onResubscribe}
                >
                  <RotateCcw className="size-4 mr-2" />
                  Resubscribe
                </Button>
              )}
            </CardContent>
          </Card>
        )}

        {(isActive || isTrial) && onCancel && (
          <div className="mt-4 flex justify-end">
            <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="text-destructive hover:text-destructive cursor-pointer">
                  <XCircle className="size-4 mr-2" />
                  Cancel Subscription
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Are you sure you want to cancel?</DialogTitle>
                  <DialogDescription>
                    Your subscription will remain active until the end of your current billing period on{" "}
                    <span className="font-medium text-foreground">{plan.nextBilling}</span>.
                    After that, you will lose access to premium features.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                  <Button
                    variant="outline"
                    onClick={() => setCancelDialogOpen(false)}
                    className="cursor-pointer"
                  >
                    No, keep my plan
                  </Button>
                  <Button
                    variant="destructive"
                    onClick={handleCancel}
                    disabled={canceling}
                    className="cursor-pointer"
                  >
                    {canceling ? "Canceling..." : "Yes, cancel subscription"}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
        )}
      </CardContent>
    </Card>
  )
}
